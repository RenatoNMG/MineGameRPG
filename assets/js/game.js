const canvas=document.querySelector("#world"),ctx=canvas.getContext("2d"),D=Math.max(1,window.devicePixelRatio||1);let W=1,H=1;function resize(){const r=canvas.getBoundingClientRect();W=Math.max(1,Math.floor(r.width));H=Math.max(1,Math.floor(r.height));canvas.width=Math.max(1,Math.floor(W*D));canvas.height=Math.max(1,Math.floor(H*D));ctx.setTransform(D,0,0,D,0,0)}addEventListener("resize",resize);resize();
