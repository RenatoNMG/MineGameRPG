...
export class Player{
  ...
  move(dx,dy,dt,canMove){if(!dx&&!dy){this.moving=false;return;}this.moving=true;const n=Math.hypot(dx,dy);if(dy!==0){this.facing="front";this.movementAxis="vertical";}else if(dx!==0){this.facing="side";this.movementAxis="horizontal";this.lastDir=dx>0?1:-1;}canMove(dx/n*this.speed*dt,dy/n*this.speed*dt);}
