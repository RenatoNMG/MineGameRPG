...
export class Player{
  ...
  move(dx,dy,dt,canMove){if(!dx&&!dy){this.moving=false;return;}this.moving=true;const n=Math.hypot(dx,dy);if(Math.abs(dx)>Math.abs(dy)){this.facing="side";this.lastDir=dx>0?1:-1;}else{this.facing="front";}canMove(dx/n*this.speed*dt,dy/n*this.speed*dt);}
