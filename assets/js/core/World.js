...
export class World{
 ...
 collectDrops(inventory,particles){for(const t of this.trees){if(t.state==="fallen"&&t.drop&&Math.hypot(this.player.x-t.drop.x,this.player.y-t.drop.y)<30){const q=t.drop.qty;inventory.add("wood","Madeira","🪵",q);particles.push({x:t.drop.x,y:t.drop.y,t:1,text:"+"+q+" MADEIRA"});t.drop=null;}}for(const s of this.stones){if(!s.collected&&Math.hypot(this.player.x-s.x,this.player.y-s.y)<32&&s.collect()){const q=1+Math.floor(Math.random()*2);inventory.add("stone","Pedra","🪨",q);particles.push({x:s.x,y:s.y,t:1,text:"+"+q+" PEDRA"});}}}
}