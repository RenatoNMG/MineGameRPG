export class ItemRenderer{
  ...
  /*
   * IMPORTANTE: este arquivo é a única fonte dos desenhos de itens.
   * Se um item ganhar um visual novo, adicione o desenho aqui e reutilize
   * o mesmo método em mundo, mão e createItemIcon. NÃO copie o desenho para
   * InventoryUI, QuickbarUI ou CraftUI.
   *
   * O createItemIcon troca temporariamente renderer.ctx. Isso é proposital
   * para reutilizar o mesmo desenho no Canvas da UI. A troca é síncrona e
   * protegida por finally; não adicionar await/callback assíncrono aqui.
   */
  /* A cerca e a porta têm UMA definição física; orientação é estado de colocação. */
  ...
  drawDroppedItem(item){
    if(item.visual==="fence"||item.visual==="fenceGate"){this.drawFence({x:item.x,y:item.y,orientation:item.orientation,visual:item.visual});return;}
    ...
    if(item.visual==="rottenMeat"){this.drawRottenMeat(c,0,0,1.25);c.restore();return;}
