export class ItemRenderer{
  ...
  drawDroppedItem(item){if(item.visual==="fence"||item.visual==="fenceGate"){this.drawFence({x:item.x,y:item.y,orientation:item.orientation,visual:item.visual,openProgress:item.openProgress});return;}if(item.visual==="wood"){this.renderer.drawWood({x:item.x,y:item.y,variant:item.variant??1,collected:false});return;}if(item.visual==="stone"){this.renderer.drawStone({x:item.x,y:item.y,type:item.type??1,radius:14,loose:true,collected:false});return;}if(item.visual==="egg"){this.renderer.drawEgg({x:item.x,y:item.y});return;}const c=this.renderer.ctx;c.save();c.translate(item.x,item.y);c.fillStyle="rgba(10,12,10,.3)";c.beginPath();c.ellipse(0,8,12,4,0,0,Math.PI*2);c.fill();if(item.visual==="rottenMeat"){this.drawRottenMeat(c,0,0,1.25);c.restore();return;}if(item.renderMode==="canvas"){c.fillStyle="#d95b5b";c.strokeStyle="#fff";c.lineWidth=2;c.strokeRect(-10,-10,20,20);c.font="bold 9px Arial";c.textAlign="center";c.textBaseline="middle";c.fillText("?",0,0);c.restore();return;}c.font="22px Arial";c.textAlign="center";c.textBaseline="middle";c.lineWidth=3;c.strokeStyle="rgba(16,20,15,.9)";c.strokeText(item.icon||"•",0,0);c.fillStyle="#fff";c.fillText(item.icon||"•",0,0);c.restore();}
}
