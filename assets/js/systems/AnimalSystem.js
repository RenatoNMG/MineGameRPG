...
export class AnimalSystem{
  ...
  updateChicken(chicken,dt){
    ...
    if(chicken.matingEggLocked){
      const nearRooster=w.roosters.some(r=>!r.carried&&Math.hypot(chicken.x-r.x,chicken.y-r.y)<50);
      if(!nearRooster)chicken.matingEggLocked=false;
    }
  ...
  moveChicken(chicken,dt){
    ...
    if(roosterThreat&&roosterThreat.matingTimer>0)return;
  ...
  wanderChicken(chicken,dt){
    ...
    if(!this.world.objectBlocks(sx,sy,13,chicken))chicken.y=sy;
  ...
  updateRooster(rooster,dt){
    ...
    const w=this.world;
    if(rooster.matingTimer>0){rooster.matingTimer-=dt;return;}
    ...
    if(target){
      ...
      if(rooster.mateTimer<=0&&best<=22&&!target.matingEggLocked&&!target.matingEggPending){
        rooster.mateTimer=7+Math.random()*5;rooster.matingTimer=1;rooster.matingCooldown=5;rooster.matingTarget=target;
