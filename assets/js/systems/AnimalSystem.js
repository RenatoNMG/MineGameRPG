...
export class AnimalSystem{
  ...
  updateRooster(rooster,dt){
    ...
    if(target){const dx=target.x-rooster.x,dy=target.y-rooster.y,len=Math.hypot(dx,dy)||1;rooster.dir=dx<0?-1:1;if(rooster.mateTimer<=0&&best<=30){rooster.mateTimer=7+Math.random()*5;rooster.matingTimer=1;rooster.matingCooldown=5;rooster.timer=.25;return;}if(best<=30)return;if(best<42){const step=Math.min(rooster.speed*1.8*dt,(best-30)*.8);const nx=rooster.x+(dx/len)*step,ny=rooster.y+(dy/len)*step;if(!w.objectBlocks(nx,ny,15,null,null)){rooster.x=nx;rooster.y=ny;rooster.dir=dx<0?-1:1;}else if(!this.moveSmart(rooster,dx/len,dy/len,dt,15,1.0,null))rooster.dir=dx<0?-1:1;}else if(!this.moveSmart(rooster,dx/len,dy/len,dt,15,1.8,null))rooster.dir=dx<0?-1:1;}else this.move(rooster,dt,15,1.8);
    rooster.x=Math.max(30,Math.min(w.width-30,rooster.x));rooster.y=Math.max(30,Math.min(w.height-30,rooster.y));
  }
