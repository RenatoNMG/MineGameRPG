...
export class CraftUI{
  ...
  render(){
    ...
    game.crafting.recipes.forEach(recipe=>{
      ...
      const itemDef=getItem(recipe.output[0]);
      ...
      b.onclick=()=>{if(game.crafting.craft(recipe)){this.q("#itemInfo").textContent="Criado: "+getItem(recipe.output[0])?.name+".";game.events.emit("crafted",{recipeId:recipe.id,output:recipe.output});this.inventoryUI.render();this.render();}};
